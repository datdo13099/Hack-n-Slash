		...
		if (collisions.size() > 0)
		{
			...
			while (!foundCollision)
			{
				...
				if (distanceBetweenTwoRects(sampleBox, collisionBox) > boxTravelSize)
				{
					...
					// sửa lỗi giá trị float nhỏ hơn 1 (vd: 0.71) bị làm tròn về số nguyên
					collisionMoveX += xMove; // cộng dồn giá trị di chuyển theo trục x
					collisionMoveY += yMove; // cộng dồn giá trị di chuyển theo trục y
					sampleBox.x = collisionMoveX; // cập nhật vị trí x của sampleBox
					sampleBox.y = collisionMoveY; // cập nhật vị trí y của sampleBox
				}
				else
				{
